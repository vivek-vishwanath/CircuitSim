...
import com.ra4king.circuitsim.simulator.components.gates.Gate.NandGate
 ...
 * @author Roi Atalla
 */
class NandGatePeer(properties: Properties, x: Int, y: Int) : GatePeer<NandGate>(properties, x, y) {
    override fun ensureProperties(properties: Properties) {
        properties.ensureProperty(Properties.BITSIZE)
        properties.ensureProperty(Properties.NUM_INPUTS)
    }

    override fun buildGate(properties: Properties): NandGate {
        val numInputs: Int
        return NandGate(
    ...
    override fun paintGate(graphics: GraphicsContext, circuitState: CircuitState?) {
        ...
        graphics.arc(x + width * 0.3, y + height * 0.5, width * 0.5, height * 0.5, 270.0, 180.0)
        graphics.closePath()
    ...
    companion object {
        ...
        fun installComponent(manager: ComponentManagerInterface) {
            manager.addComponent(
                Pair("Gates", "NAND"),
                Image(NandGatePeer::class.java.getResourceAsStream("/images/NandGate.png")),
                Properties(), true
            )
        }
    }
