 ...
 * @author Roi Atalla
 */
class XorGatePeer(properties: Properties, x: Int, y: Int) : GatePeer<XorGate>(properties, x, y) {
    override fun ensureProperties(properties: Properties) {
        properties.ensureProperty(Properties.BITSIZE)
        properties.ensureProperty(Properties.NUM_INPUTS)
    }

    override fun buildGate(properties: Properties): XorGate {
        val numInputs: Int
        return XorGate(
    ...
    companion object {
        ...
        fun installComponent(manager: ComponentManagerInterface) {
            manager.addComponent(
                Pair("Gates", "XOR"),
                Image(XorGatePeer::class.java.getResourceAsStream("/images/XorGate.png")),
                Properties(), true
            )
        }
    }
